...
const Furnitures = () => {
  ...
  const furnitureCategoryId = 37; 

  useEffect(() => {
    axios
      .get("https://api.escuelajs.co/api/v1/products")
      .then((response) => {
        // Filter products by Furniture category ID
        const filteredProducts = response.data.filter(
          (product) => product.category.id === furnitureCategoryId
        );
        setFurnitureProducts(filteredProducts);
        setLoading(false);
      })
      .catch((error) => {
